...
public class FitnessFunction {
    ...
    public int dimension = MA.nCloudlets;
    private int minVal = 0;
    private int maxVal = MA.nTotalVms;
    ...
    public double getFitnessValue(int[] results, List<Vm> vmList, List<Cloudlet> cloudletList) {
        ...
        double mean = totalLength / MA.nTotalVms;

        double sum = 0;

        for (int i = 0; i < MA.nTotalVms; i++) {

            sum += Math.pow((relativeRatio[i] - mean), 2);

        }
        double std = sum / MA.nTotalVms;
