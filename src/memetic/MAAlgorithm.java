...
import MyThesis.SimulatedAnnealing;
 ...
 * @author Maziyar
 */
public class MAAlgorithm {

    private static int NUM_CHROMOSOMES = 50;
    private static int CHROMOSOMES_LENGTH = MA.nCloudlets;
    ...
    private static Results start() {
        ...
        for (int numRuns = 0; numRuns < maxIt; numRuns++) {
            ...
            SimulatedAnnealing sa = new SimulatedAnnealing(population.get(0), vmList, cloudletList);
...
class Chromosome implements Comparable {
    ...
    protected int[] position = new int[MA.nCloudlets];
    ...
    public Chromosome(int length, List<Vm> vmList, List<Cloudlet> cloudletList) {
        ...
        public Chromosome(int [] position,  List<Vm> vmList, List<Cloudlet> cloudletList) {
  
        genetic.FitnessFunction fitFunc = new genetic.FitnessFunction();
        cost = fitFunc.getFitnessValue(position,vmList, cloudletList);
    ...
    public int randomFill() {
        Random rand = new Random();
        int randomNum = rand.nextInt(MA.nTotalVms);
        return randomNum;
    }
